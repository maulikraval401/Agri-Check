  ...
  return (
    <div className="page-enter">
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <section className="rounded-[1.35rem] border border-[hsl(var(--border))] bg-[hsl(var(--card)/.62)] p-5 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="eyebrow">{copy.recent}</p>
              <h2 className="mt-2 text-xl font-bold tracking-[-.03em]">
                {history.length} {copy.tests}
              </h2>
            </div>
            <Link href="/history" className="flex min-h-10 items-center gap-1 rounded-lg px-2 text-sm font-bold text-[hsl(var(--primary))]">
              {copy.viewHistory}
              <ChevronRight size={16} />
            </Link>
          </div>
          {history.length === 0 ? (
            <p className="mt-8 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{copy.noHistoryBody}</p>
          ) : (
            <div className="mt-5 space-y-3">
              {history.slice(0, 3).map((test) => (
                <div key={test.id} className="flex items-center justify-between rounded-xl bg-[hsl(var(--muted)/.55)] p-3">
                  <div>
                    <p className="text-sm font-bold">{formatDate(test.createdAt, language)}</p>
                    <p className="mt-1 font-mono-field text-[.65rem] text-[hsl(var(--muted-foreground))]">
                      {Object.values(test.nutrients).filter((item) => item.status === 'Low').length} {copy.lowMarkers}
                    </p>
                  </div>
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--primary))]">
                    <ChevronRight size={16} />
                  </span>
                </div>
              ))}
  ...
  return (
    <div className="page-enter">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="eyebrow">{copy.archiveLabel}</p>
          <h1 className="mt-2 text-3xl font-bold tracking-[-.05em]">{copy.historyTitle}</h1>
          <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{copy.historyBody}</p>
        </div>
        {history.length > 0 && (
          <button
            type="button"
            onClick={onClear}
            className="flex min-h-11 items-center gap-2 rounded-xl border border-[#e5b5a5] px-4 text-sm font-bold text-[#9c4936]"
          >
      ...
      <div className="mt-7">
        {history.length === 0 ? (
          <EmptyHistory onStart={onStart} />
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {history.map((test, index) => (
              <article
                key={test.id}
                className="rise-in rounded-[1.35rem] border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-4 shadow-[var(--shadow-soft)] sm:p-5"
                style={{ animationDelay: `${index * 60}ms` }}
              >
                <div className="flex gap-4">
                  {test.imageDataUrl ? (
                    <img src={test.imageDataUrl} alt="Saved soil sample" className="h-20 w-20 rounded-xl object-cover" />
                  ) : (
                    <div className="grid h-20 w-20 shrink-0 place-items-center rounded-xl bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))]">
                      <FileImage size={23} />
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="font-mono-field text-[.65rem] uppercase tracking-[.1em] text-[hsl(var(--muted-foreground))]">
                      {formatDate(test.createdAt, language)}
                    </p>
                    <h2 className="mt-2 truncate text-lg font-bold">{test.recommendation.split('.')[0]}.</h2>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {(Object.keys(test.nutrients) as NutrientKey[]).map((key) => (
                        <span key={key} className="rounded-md bg-[hsl(var(--muted))] px-2 py-1 text-[.67rem] font-bold">
                          {nutrientMeta[key].short}: {test.nutrients[key].status}
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => onOpen(test)}
                  className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary)/.09)] text-sm font-bold text-[hsl(var(--primary))]"
                >
                  {copy.view}
                  <ArrowRight size={16} />
                </button>
